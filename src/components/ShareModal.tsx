...
export default function ShareModal({ url, visitedCount, onClose, onRegenerate }: ShareModalProps) {
  ...
  const text = `I've explored ${visitedCount} ${visitedCount === 1 ? 'country' : 'countries'} 🌍 — where are you going next? Start your own travel log!`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4" onClick={onClose}>
      <div
        className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-sm space-y-5 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-xl font-bold text-white">Share your travel map</h2>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {platforms.map((p) => (
              <a
                ...
                className={`${p.color} rounded-xl p-3 flex flex-col items-center gap-1.5 hover:opacity-90 transition-opacity`}
              >
          ...
          <p className="text-xs text-gray-500 mt-3 text-center">
            For Instagram & TikTok, use <span className="text-gray-300">"Share via…"</span> on mobile or copy the link
          </p>
        </div>
      </div>
    </div>
